...
public static class Armageddon
{
  ...
  private static void Giants_Mountains(ZPerson p)
  {
    ...
    else
    {
      ...
      int x = p.DequeueArmaWarning().x;
      ZSpell.FireVolcano(Inert.Instance.ArmageddonObjects[GameFacts.GetMapIndex(p.game.armageddon)], p.controlled[0], new MyLocation(x, -300), true);
      p.AddArmaWarning(-35, 0);
    }
  }
  ...
  private static void Elven_Isles(ZPerson p)
  {
    if (p.armaWarnings.Count == 0)
    {
      p.AddArmaWarning(p.map.Height + 45, 0);
    }
    else
    {
      int x = p.DequeueArmaWarning().x;
      ZSpell.FireCometArmageddon(Inert.Instance.ArmageddonObjects[GameFacts.GetMapIndex(p.game.armageddon)], p, new MyLocation(x, 50));
      p.AddArmaWarning(p.map.Height + 45, 0);
    }
  }
  ...
  private static void Murkey_Swamp(ZPerson p)
  {
    ...
    {
      int x = p.DequeueArmaWarning().x;
      ZSpell.FireStorm(Inert.Instance.ArmageddonObjects[GameFacts.GetMapIndex(p.game.armageddon)], p.controlled[0], new MyLocation(x, 0), 300, true);
      p.AddArmaWarning(p.map.Height + 45, 0);
    }
  }
  ...
  private static void Graveyard(ZPerson p)
  {
    ...
    {
      int x = p.DequeueArmaWarning().x;
      ZSpell.FireRainOfChaos(Inert.Instance.ArmageddonObjects[GameFacts.GetMapIndex(p.game.armageddon)], p.first(), new MyLocation(x, 0), true);
      p.AddArmaWarning(p.map.Height + 45, 0);
    }
  }
  ...
  private static void Desert(ZPerson p)
  {
    ...
    else
    {
      ...
      p.lastArmageddon = p.localTurn;
      int x = p.DequeueArmaWarning().x;
      ZSpell.FireBurningSands(Inert.Instance.ArmageddonObjects[GameFacts.GetMapIndex(p.game.armageddon)], p.first(), new MyLocation(x, 0), 25, true);
      p.AddArmaWarning(-35, 0);
    }
  }
  ...
  private static void Sky_Castles(ZPerson p)
  {
    ...
    else
    {
      ...
      p.game.ongoing.RunSpell(ZSpell.FireSkyRay(armageddonObject, p.first(), new MyLocation(x, 0), true), true);
      if (p.game.isClient)
  ...
  private static void Mos_LeHarmless(ZPerson p)
  {
    ...
    else
    {
      int x = p.DequeueArmaWarning().x;
      Spell armageddonObject = Inert.Instance.ArmageddonObjects[GameFacts.GetMapIndex(p.game.armageddon)];
      ZSpell.FireSummon(armageddonObject, p.game, p.controlled[0], new MyLocation(x, p.map.Height + armageddonObject.radius + 50), -1, false, (ZPerson) null);
      p.AddArmaWarning(p.map.Height + 45, 0);
    }
  }
  ...
  private static void Arcane_Crystals(ZPerson p)
  {
    ...
    else
    {
      ...
      MyLocation target = new MyLocation(x, -300);
      ...
      else
      {
        ...
        if (zcreatureList.Count > 0)
        {
          ...
          target = new MyLocation(p.map.Width - x, p.game.RandomInt(200, 500));
      }
      p.game.ongoing.RunSpell(ZSpell.IEnumeratorArcaneArrows(armageddonObject, p.controlled[0], new MyLocation(x, 0), Quaternion.identity, (FixedInt) 90, (FixedInt) 1, amount, target, p.controlled[0].HasArcaneEnergizer ? (int) ((FixedInt) armageddonObject.damage * (FixedInt) 1572864L) : 0, true), true);
      p.AddArmaWarning(-35, 0);
    }
  }
  ...
  private static void Ocean_Floor(ZPerson p)
  {
    ...
    {
      ZSpell.FireRainOfClams(Inert.Instance.clamOceanFloor, p.first(), new MyLocation(p.DequeueArmaWarning().x, 50), Mathf.Max(1, p.localTurn / 10), true, true);
      p.AddArmaWarning(p.map.Height + 45, 0);
      if (p.localTurn % 5 != 1)
        return;
      p.lastArmageddon = p.localTurn;
  ...
  private static void Wasteland(ZPerson p)
  {
    ...
    if (p.localTurn == 25 && p.game.armageddonTurnVariable != 25)
    {
      ...
      ZSpell.FireRedo(spell, c, true, false);
      if (p.game.isClient)
        AudioManager.Play(spell.castClip);
    }
    if (p.armaWarnings.Count == 0)
