...
public class MapEditor : MonoBehaviour
{
  ...
  {
    get => this.layers.FindIndex((Predicate<ZMap>) (z => z == this.selectedLayer));
  }

  internal static float getObjectRot => MapEditor.Instance.input_objectRot.text.ToFloat();

  internal static float getObjectScale => MapEditor.Instance.input_objectScale.text.ToFloat();

  internal static bool getObjectFlipX => MapEditor.Instance.toggle_objectFlipX.isOn;

  internal static bool getObjectBehind => MapEditor.Instance.toggle_objectBehind.isOn;

  internal static float getObjectHue => MapEditor.Instance.input_objectHue.text.ToFloat();

  internal static Color32 getTextureColor => (Color32) MapEditor.Instance.img_textureColor.color;

  internal static Color32 getObjectColor => (Color32) MapEditor.Instance.img_objectColor.color;

  internal static bool getTextureIgnoreExisting
  {
    get => MapEditor.Instance.toggle_textureIgnoreExisting.isOn;
  }

  internal static float getTextureHue => MapEditor.Instance.input_textureHue.text.ToFloat();

  public Texture2D _tex
  {
    get => this._texPrivate;
    set
    {
      this.DestroyTex();
  ...
  private void Awake()
  {
    ...
    Controller.Instance.InitMap(false, openHUD: false);
    this.NewMap();
  }
    ...
    for (int index = 0; index < this.layers.Count; ++index)
    {
      ...
      g.onRightClick.AddListener((UnityAction) (() =>
      {
        ...
        myContextMenu.AddInput((Action<string>) (s =>
        {
          ...
          this.RefreshUI();
        }), g.transform.GetChild(0).GetComponent<TMP_Text>().text);
  ...
  public void OnToggleObjectFlipX(bool v) => this.sr_object.flipX = v;
  ...
  {
    float num = Mathf.Clamp(s.ToFloat(), 0.1f, 10f) * 100f;
    this.sr_object.transform.localScale = new Vector3(num, num, 1f);
    this.slider_objectScale.SetValueWithoutNotify(s.ToFloat());
  }

  public void OnEditObjectHue(string s)
  {
    float r = Mathf.Clamp(s.ToFloat(), 0.0f, 1f);
    this.sr_object.color = new Color(r, 1f, 1f, 0.5f);
    this.slider_objectHue.SetValueWithoutNotify(s.ToFloat());
  ...
  public void OnEditTextureHue(string s)
  {
    ...
    this.slider_textureHue.SetValueWithoutNotify(s.ToFloat());
    this.input_textureHue.SetTextWithoutNotify(r.ToString());
  }

  public void OnEditTextureBrushSize(string s)
  {
    this.brushSize = Mathf.Clamp(s.ToInt(), 1, 2048);
    this.input_tex_brushSize.SetTextWithoutNotify(this.brushSize.ToString());
  }

  public void MenuStrip_BoxSelection() => this.OnToolChange(MapEditor.Tool.BoxSelect);
  ...
  public void MenuStrip_File()
  {
    ...
    myContextMenu.AddItem("Open Arcpng", (Action) (() => MyFilePicker.Create("Open Arc Map", "CustomMaps", ".arcpng", false, new Action<string>(this._open), ContentType.ArcMap)), MyContextMenu.ColorGreen);
    myContextMenu.AddItem("Open png", (Action) (() => MyFilePicker.Create("Open png", "CustomMaps", ".png", false, new Action<string>(this._open), ContentType.ArcMap)), MyContextMenu.ColorGreen);
    myContextMenu.AddItem("Save Arcpng", (Action) (() => MyFilePicker.Create("Save Arc Map", "CustomMaps", ".arcpng", true, (Action<string>) (s => this._save(s, ".arcpng")), ContentType.ArcMap)), MyContextMenu.ColorGreen);
    myContextMenu.AddItem("Save png", (Action) (() => MyFilePicker.Create("Save png", "CustomMaps", ".png", true, (Action<string>) (s => this._save(s, ".png")), ContentType.ArcMap)), MyContextMenu.ColorGreen);
    myContextMenu.AddSeperator();
    myContextMenu.AddItem("Quit", (Action) (() =>
    {
      if (this.stack_undo.Count > 0 || this.stack_redo.Count > 0)
        MyMessageBox.Create("Are you sure you want to quit?", (Action) (() => Controller.Instance.OpenMenu(Controller.Instance.MenuTutorialCodeEditor, false)));
      else
  ...
  internal void _open(string file)
  {
      ...
      else
      {
        using (MemoryStream memoryStream = new MemoryStream(File.ReadAllBytes(file)))
        {
          using (myBinaryReader myBinaryReader = new myBinaryReader((Stream) memoryStream))
          {
            ...
            this.mapSizeX = num1 >= 200 && num2 >= 10 && num1 <= 8192 && num2 <= 8192 ? num1 : throw new Exception("Map Size is too big/small");
            this.mapSizeY = num2;
    ...
    catch (Exception ex)
    {
      ...
      this.CleanUp();
      MyMessageBox.Create("Error loading image: " + ex.ToString(), (Action) null, cancel: (string) null);
    }
    this.OnNewMap();
  }
  ...
  {
    MyContextMenu myContextMenu = MyContextMenu.Show();
    InputFieldPlus width = myContextMenu.AddInput((Action<string>) null, this.mapSizeX.ToString(), hasButton: false);
    InputFieldPlus height = myContextMenu.AddInput((Action<string>) null, this.mapSizeY.ToString(), hasButton: false);
    myContextMenu.AddItem("Generate", (Action) (() =>
    {
      Prefs.mapEditorWidthX = width.inputText.text.ToInt();
      Prefs.mapEditorWidthY = height.inputText.text.ToInt();
      this.NewMap();
    }), MyContextMenu.ColorGreen);
    myContextMenu.Rebuild();
  }
  ...
  {
    ColorPickerUI.Create(this.img_textureColor.color with
    {
      a = 1f
    }, (Action<Color>) (c => this.img_textureColor.color = c), (Action<Color>) (c => this.img_textureColor.color = c), (Action) (() => this.img_textureColor.color = Color.clear));
  }

  public void SubMenu_ObjectColor()
  {
    ColorPickerUI.Create(this.img_objectColor.color with
    {
      a = 1f
    }, (Action<Color>) (c => this.img_objectColor.color = c), (Action<Color>) (c => this.img_objectColor.color = c), (Action) (() => this.img_objectColor.color = Color.clear));
  }
  ...
  public void MenuStrip_ClickGrass()
  {
    ...
    {
      MyContextMenu myContextMenu2 = MyContextMenu.Show();
      myContextMenu2.AddInput((Action<string>) (s => this.grass_offset = s.ToInt(6)), this.grass_offset.ToString());
      myContextMenu2.Rebuild();
    }), MyContextMenu.ColorGreen);
    myContextMenu1.Rebuild();
  }
  ...
  public ZMap InsertLayer(int index, string name)
  {
    ...
    for (int index1 = index; index1 < this.layers.Count; ++index1)
      this.UpdateLayerOrder(this.layers[index1], index1);
  ...
  public void RemoveLayer() => this.RemoveLayer(true);
  ...
  private void Update()
  {
    if (Input.GetKeyDown(KeyCode.Escape))
      this.MenuStrip_File();
    else if (Player.IsPointerOverGameObject() && !Input.GetMouseButton(0) && Input.touchCount <= 0)
    {
      this.softLock = true;
    }
    else
  ...
  public class BoxSelection
  {
    ...
    {
      int x1 = Mathf.Min(this.start.x, this.end.x);
      int x2 = Mathf.Max(this.start.x, this.end.x);
      int y1 = Mathf.Max(this.start.y, this.end.y);
      int y2 = Mathf.Min(this.start.y, this.end.y);
      return new Vector3[4]
      {
        new Vector3((float) x1, (float) y1),
        new Vector3((float) x2, (float) y1),
        new Vector3((float) x2, (float) y2),
        new Vector3((float) x1, (float) y2)
      };
    }

    public int width => this.end.x - this.start.x;

    public int height => this.start.y - this.end.y;
  ...
  public class UndoLayerSelect : MapEditor.IUndoOperation
  {
    ...
    public UndoLayerSelect(int layer) => this.layer = layer;
  ...
  public class UndoLayerMove : MapEditor.IUndoOperation
  {
    ...
    public UndoLayerMove(bool up) => this.up = up;
  ...
  public class UndoPixels : MapEditor.IUndoOperation
  {
    ...
    {
      MapEditor.UndoPixels changed = p ?? new MapEditor.UndoPixels();
      ...
      {
        if (rawSprites[i].requireUpdate && changed.areas.Find((Predicate<MapEditor.UndoPixels.area>) (t => t.layer == layer && t.index == i)) == null)
          changed.Add(layer, i, rawSprites[i].sr.sprite.texture.GetPixels32());
      }
      return changed;
    }
