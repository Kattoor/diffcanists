...
public class UnratedMenu : Catalogue
{
  ...
  public void RefreshNames(bool setReadyStates = true, bool updatePos = false)
  {
    ...
    for (; index1 < Client._gameFacts.players.Count; ++index1)
    {
      ...
      ((RectTransform) andApply.transform).anchoredPosition = new Vector2((float) num2, (float) (-index1 * num1 - 2));
      andApply.GetComponent<pfabName>().Setup(Client.GetAccount(Client._gameFacts.players[index1], false), UnratedMenu.readyStates != null && UnratedMenu.readyStates.Length > index1 && UnratedMenu.readyStates[index1]);
    }
    int num3 = index1;
  ...
  private void UpdateStartButton()
  {
    ...
    this.buttonStartGame.interactable = flag1 ? Client._gameFacts.players.Count % Client._gameFacts.GetNumberPlayersPerTeam() == 0 && Client._gameFacts.players.Count > Client._gameFacts.GetNumberPlayersPerTeam() : flag2;
    this.SetStartButton();
  }
  ...
  {
    MyToolTip.Show("Countdown: " + Mathf.Abs((int) Client._gameFacts.countdownTime).ToString() + (Client._gameFacts.countdownTime > (short) 0 ? " seconds<br>If time runs out you will get 5 second turns from then on." : " seconds<br><#FF0000>If time runs out you lose</color><br>Turn time is still used."), -1f, false);
  }
  ...
  public void ClickDescription()
  {
    ...
    myContextMenu.AddInput((Action<string>) (s => Client.AskToChangeDescription(s)), Client._gameFacts.settings.description, true, true);
    if (Client._gameFacts.GetRatedMode())
