...
public class ZGame
{
  ...
  private static string[] shortNames = new string[102]
  {
    "Leo",
    "Max",
    "Eli",
  ...
  private static List<string> replacedNames = new List<string>();
  ...
  {
    SpellLobbyChange.Create(Client.settingsPlayer, (Action<SettingsPlayer>) (set => Client.AskBanStage(set)), false, this.curBanStage == BanStage.Pick ? Validation.Level3Only : Validation.None, true, (Action) (() => this.init_BanStage()), false, (Restrictions) null);
  }
...
label_57:
  ...
  public void OnEndTurn(ZPerson p)
  {
    ...
    for (int index = p.controlled.Count - 1; index >= 0; --index)
    {
      if (index < p.controlled.Count)
      {
        if (p.controlled[index].tempFlight)
          p.controlled[index].RemoveFlight(false, true);
    ...
    if (zcreature.inWater && !zcreature._sandsOfTime.HasValue && (p.localTurn > 0 || !this.First_Turn_Teleport))
    {
      if (this.isServer)
      {
        ...
        {
          zcreature.DoDamage(5 * p.waterMultipler, DamageType.None, (ZCreature) null, false, (ISpellBridge) null);
    ...
    else
    {
      if (!this.zombieCharge.HasValue)
        return;
      ZSpell.FireCallOfTheDead(Inert.GetSpell(SpellEnum.Call_of_the_Dead), p.first(), this.zombieCharge.Value, 0, false);
  ...
  public void SendCreatureHealthInstant(ZCreature c)
  {
    using (MemoryStream memoryStream = new MemoryStream())
    {
      using (myBinaryWriter myBinaryWriter = new myBinaryWriter((Stream) memoryStream))
      {
        myBinaryWriter.Write((byte) 184);
        myBinaryWriter.Write(c.parent.id);
        myBinaryWriter.Write(c.parent.controlled.IndexOf(c));
        myBinaryWriter.Write(c.health);
      }
...
label_46:
  ...
  public void NextTurn(ZPerson p, bool first = true)
  {
    ...
    this.PlayersMaxTurnTime = p.GetLevel(BookOf.Cogs, false) > 0 ? (float) this.MaxTurnTime + (float) p.GetLevel(BookOf.Cogs, false) * Mathf.Min(this.originalSpellsOnly ? 6f : (float) this.MaxTurnTime / 5f, 6f) : (float) this.MaxTurnTime;
    if (this.isCountdown)
    {
  ...
  public void NextTurn(ZCreature x)
  {
    ...
    {
      x.DoDamage(5, DamageType.None, (ZCreature) null, false, (ISpellBridge) null);
      if (x.health <= 0)
        x.health = 1;
    }
    else if (x.type == CreatureType.Vampire && x.health > 1 && x.InSunlight())
    {
      x.DoDamage(Mathf.Min(x.health - 1, 10), DamageType.None, (ZCreature) null, false, (ISpellBridge) null);
      if (x.health <= 0)
        x.health = 1;
    }
    if (x.halfHealing > 0)
        ...
        using (myBinaryWriter myBinaryWriter = new myBinaryWriter((Stream) memoryStream))
        {
          ...
          if (list.Count > 0)
          {
            ...
            if (zgame.isTeam)
            {
              try
              {
                int index2 = zgame.GetTeam(zgame.players[(int) zgame.serverState.playersTurn].team).FindIndex(new Predicate<ZPerson>(zgame.\u003CBidUpdate\u003Eb__198_1));
                if (index2 > 0)
                {
  ...
  public void CreateFamiliar(BookOf b00k, ZPerson p, bool CREATE_FAMILIAR = true, bool fromReplay = false)
  {
    ...
    switch (b00k)
    {
      case BookOf.Seasons:
        ...
        if (p.seasonISHoliday)
        {
          ...
          if (this.isClient && (UnityEngine.Object) Player.Instance != (UnityEngine.Object) null && p == Player.Instance.person)
            Player.Instance.UnselectSpell();
          double num = (double) ((FixedInt) p.GetLevel(BookOf.Seasons, false) * (FixedInt) 62914L + 1).ToFloat();
      ...
      case BookOf.Illusion:
        ...
        double num1 = (double) ((FixedInt) 1 - (FixedInt) ((long) p.GetLevel(BookOf.Illusion, false) * 125829L)).ToFloat();
        zcreature4.CalculateMass();
        zcreature4.CalculateSize();
        this.forceRysncPause = true;
    ...
    {
      if (familiar.bookOf != BookOf.Underdark || p.GetLevel(b00k, false) <= 0)
        return;
    ...
    else
    {
      ...
      {
        if (zfamiliar.bookOf != BookOf.Frost || p.GetLevel(BookOf.Frost, false) != 1 || !p.InWater() || p.localTurn <= 0 && this.First_Turn_Teleport)
          return;
        ZCreature zcreature3 = p.first();
      ...
      using (myBinaryReader myBinaryReader = new myBinaryReader((Stream) memoryStream1))
      {
        try
        {
          ...
          if (num1 > (byte) 200)
          {
            ...
            if (((!this.isClient || num1 >= (byte) 220 ? 0 : (num1 != (byte) 210 ? 1 : 0)) & (fromServer ? 1 : 0) & (ignoreOwn ? 1 : 0)) != 0 && (UnityEngine.Object) Player.Instance != (UnityEngine.Object) null && p == Player.Instance.person)
            {
              if (num1 != (byte) 207)
                return;
            ...
            else
            {
              switch (num1)
              {
                ...
                case 209:
                  ...
                  break;
                case 210:
                  if (!this.gameFacts.GetStyle().HasStyle(GameStyle.Dynamic) || !p.CanSwapSpells())
                    break;
                  byte i = myBinaryReader.ReadByte();
                  if (this.gameFacts.restrictions != null && Restrictions.IsSpellRestricted(p.settingsPlayer._spells, (int) i, this.gameFacts.restrictions))
                  {
                    if (!this.isClient)
                      break;
                    MyToolTip.Show("This spell is restricted", -1f, false);
                    break;
                  }
                  for (int index = 0; index < p.settingsPlayer.spells.Length; ++index)
                  {
                    if (p.settingsPlayer.spells[index] == byte.MaxValue && (int) i < Inert.Instance.altSpells.Length)
                    {
                      ...
                      if (spell.level == 2)
                      {
                        ...
                        HUD.OnInitSpell(p.first(), p.first().spells[p.first().spells.Count - 1], false, true);
                        if (!p.game.isClient)
                          break;
                        ...
                        else
                        {
                          ...
                          else
                          {
                            ...
                            if (creature.inWater)
                            {
                              if (!this.First_Turn_Teleport || creature.parent.localTurn > 0)
                              {
                                if ((spell.spellEnum == SpellEnum.Arcane_Gate || spell.spellEnum == SpellEnum.Santas_Magic) && (spellSlot.LastTurnFired > localTurn - 5 && spellSlot.LastTurnFired > -1 || creature.parent.arcaneGateSpellSlot == null) && this.isServer)
                                {
                                  ...
                                  {
                                    creature.DoDamage(5, DamageType.None, (ZCreature) null, false, (ISpellBridge) null);
                                    this.SendCreatureHealthInstant(creature);
                                    this.MoveQue.Enqueue((Action) (() =>
                                    {
                            ...
                            else
                            {
                              ...
                              {
                                if (!spellSlot.WillEndTurn(creature) || spellSlot.RechargeTime > 0)
                                {
                                  spellSlot.SetTurnFired = localTurn;
                                  creature.OnSpellFired(spellSlot);
                                }
                            ...
                            if (spellSlot.WillEndTurn(creature))
                            {
                              this.serverState.busy = ServerState.Busy.Between_Turns;
                              p.yourTurn = false;
                            }
                            ...
                            if (spellSlot.WillEndTurn(creature) && !this.resyncing)
                            {
                              if (!this.isSandbox && !this.isTutorial)
                            ...
                            if (this.isClient && !this.resyncing && (spellSlot.WillEndTurn(creature) || (UnityEngine.Object) Player.Instance == (UnityEngine.Object) null || !Player.Instance.person.yourTurn))
          ...
          else if (this.isClient)
          {
            switch (num1)
            {
              ...
              case 13:
                ...
                if (this.isReplay && Global.GetPrefBool("annon", false))
                {
                  for (int index = 0; index < ZGame.replacedNames.Count; ++index)
                    msg = msg.Replace(ZGame.replacedNames[index], this.gameFacts.players[index]);
                }
                foreach (ZPerson player in this.players)
...
label_188:
              ...
              case 184:
                ...
                if (p.controlled.Count <= index13)
                  break;
                p.controlled[index13].health = num16;
                p.controlled[index13].UpdateHealthTxt();
                break;
              case 185:
                int index14 = myBinaryReader.ReadInt32();
                MyLocation myLocation1 = myBinaryReader.ReadMyLocation();
                p.controlled[index14].effectors.Find((Predicate<ZEffector>) (z => z.type == EffectorType.Sands_of_Time)).target = myLocation1;
                break;
              case 186:
              ...
              case 192:
                ...
                if (index16 == -1 || !((ZComponent) this.players[index16].first() != (object) null) || this.players[index16].bid <= 0)
                  break;
                this.players[index16].first().health -= this.players[index16].bid;
                this.players[index16].first().UpdateHealthTxt();
                break;
              case 193:
              case 195:
                if (this.isMulti)
          ...
          else
          {
            switch (num1)
            {
              ...
              case 157:
                ...
                myBinaryReader.ReadInt32();
                if (index17 < 0 || index17 >= this.players.Count)
                  break;
                ...
                ZPerson player1 = this.players[index17];
    ...
    try
    {
      using (FileStream fileStream = new FileStream(file, FileMode.Open, FileAccess.Read, FileShare.None))
      {
        using (myBinaryReader reader = new myBinaryReader((Stream) fileStream))
        {
          ...
          {
            List<string> list = ((IEnumerable<string>) ZGame.shortNames).ToList<string>();
            ZGame.replacedNames.Clear();
            for (int index1 = 0; index1 < gameFacts.players.Count; ++index1)
            {
              ZGame.replacedNames.Add(gameFacts.players[index1]);
              int index2 = UnityEngine.Random.Range(0, list.Count);
              gameFacts.players[index1] = list[index2];
              list.RemoveAt(index2);
              CharacterCreation.Randomize(gameFacts.settingsPlayer[index1]);
            }
          }
  ...
  public IEnumerator<float> PushReplay()
  {
    ...
    HUD.instance.replayTimeline._bar.onValueChanged.AddListener(new UnityAction<float>(game.\u003CPushReplay\u003Eb__268_0));
  ...
  private void _Deserialize(myBinaryReader reader)
  {
    ...
    foreach (ZPerson player in this.players)
    {
      ...
      foreach (ArmaWarning armaWarning in player.armaWarnings)
